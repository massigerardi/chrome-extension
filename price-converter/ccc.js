...
CCC = {
	...
	addEvents	: function() {
		...
		if(this.mini) {
			window.addEventListener('keydown', function(event) {
				...
				// Toggle CCC.Mini
				if(CCC.keys.alt && (CCC.keys.shift || navigator.platform.indexOf('Mac') === 0)  && CCC.keys.ctrl && event.keyCode === 67) {
	...
	parse	: function(parent, noElement) {
		...
		if(!parent || parent._CCC || /^(G|CANVAS|PATH|IFRAME|CIRCLE|RECT|SVG|EMBED|OBJECT|VIDEO|AUDIO|CCC|IMG|STYLE|SCRIPT|IFRAME|CCC|PRE)$/.test(parent.nodeName) || parent.childNodes.length == 0) {
			return false;
		}
